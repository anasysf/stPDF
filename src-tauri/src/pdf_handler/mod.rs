    ...
    P: AsRef<Path> + Debug + Display,
{
    ...
    fn new_page(
        ...
        let (new_page, layer_idx) =
            PdfPage::new(Mm(width), Mm(height), layer_name, page_idx.unwrap_or(self.pages.len()));
...
pub(crate) fn generate_pdfs_from_sources<P: AsRef<Path> + Debug + Display>(
        ...
        match image::open(&source) {
            Ok(image) => {
                ...
                let event_payload = format!(
                    "Decoding: {}",
                    img_file_name.unwrap_or(OsStr::new("Image")).to_str().unwrap_or("Image")
                );

                app_handle.emit_to(EventTarget::App, "decoding-barcode", event_payload).unwrap();
    ...
    for scanned_document in scanned_documents {
        let ScannedDocumentParent { id, doc, page_idx, layer_idx, image, joints } =
            scanned_document;
        ...
        app_handle.emit_to(EventTarget::App, "generating-pdf", event_payload).unwrap();
    ...
    app_handle.emit_to(EventTarget::App, "done-generating", "Done").unwrap()
}
